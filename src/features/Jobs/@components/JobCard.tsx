    ...
    return (
        <Card className="h-full border-gray-200 hover:border-blue-300 transition-colors shadow-sm hover:shadow-md flex flex-col">
            <CardHeader className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                ...
                {job?.is_recent && (
                    <Badge variant="secondary" className="bg-green-100 text-green-800 flex-shrink-0 mt-2 sm:mt-0">New</Badge>
                )}
            </CardHeader>
            ...
            <CardFooter className="flex flex-col sm:flex-row items-start sm:items-center justify-between border-t pt-4 mt-auto gap-4">
                <div className="font-semibold text-blue-600 w-full sm:w-auto">
                    {job?.salary_range ?? "Salary Range"}
                </div>
                <Button className={cn(blueTheme.accent, blueTheme["primary-foreground"], "w-full sm:w-auto")}
